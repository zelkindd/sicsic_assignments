...
void add_warehouse(wlst **head, char *name, int code) {
    ...
    if (*head == NULL || (*head)->data->code > code) {
        new_node->next = *head;
        *head = new_node;
    } else {
        wlst *temp = *head;
        while (temp->next != NULL && temp->next->data->code < code)
            temp = temp->next;
        new_node->next = temp->next;
        temp->next = new_node;
    }
...
void assign_item_to_warehouse(itemlst *item_head, wlst *w_head, int item_id, int w_code) {
    ...
    if (target_item == NULL || target_w == NULL)
    ...
    if (target_item->warehouses == NULL) {
        ...
        wlst *temp = target_item->warehouses;
        while (temp->next != NULL)
            temp = temp->next;
        temp->next = new_w_node;
    }
    ...
    if (target_w->items == NULL) {
        ...
        itemlst *temp = target_w->items;
        while (temp->next != NULL)
            temp = temp->next;
        temp->next = new_i_node;
    }
}
